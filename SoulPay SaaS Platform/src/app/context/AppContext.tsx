...
function getStoredTemplateMap(): Record<string, string> {
  if (typeof window !== 'undefined') {
    try {
      const saved = localStorage.getItem('soulpay_tenant_templates') || localStorage.getItem('faithpay_tenant_templates');
...
function saveStoredTemplate(key: string, templateId: string) {
  if (typeof window !== 'undefined' && key && templateId) {
    try {
      ...
      localStorage.setItem('soulpay_tenant_templates', JSON.stringify(currentMap));
    } catch (e) {}
  }
}
