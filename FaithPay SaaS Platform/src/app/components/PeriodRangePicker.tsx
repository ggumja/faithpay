...
export function PeriodRangePicker({
  ...
  // Open modal handler
  const handleOpen = () => {
    setTempStartDate(selection.startDate);
    setTempEndDate(selection.endDate);
    setPickerYear(selection.startDate.getFullYear());
    setPickerMonth(selection.startDate.getMonth());
  ...
  const handleUnitSwitch = (newUnit: PeriodUnit) => {
    ...
    if (newUnit === 'daily') {
      ...
      label = `${start.getFullYear()}년 1월 ~ ${start.getFullYear()}년 6월`;
  ...
  const handleApply = () => {
    ...
    const start = tempStartDate < finalEnd ? tempStartDate : finalEnd;
    const end = tempStartDate < finalEnd ? finalEnd : tempStartDate;
  ...
  const handleDayClick = (d: Date) => {
    if (selectionStep === 'start' || !tempStartDate || (tempStartDate && tempEndDate)) {
      setTempStartDate(d);
      setTempEndDate(null);
  ...
  const isSelectedDay = (d: Date) => {
    if (!d) return false;
    const t = d.getTime();
    if (tempStartDate && t === tempStartDate.getTime()) return true;
    if (tempEndDate && t === tempEndDate.getTime()) return true;
    return false;
  };

  const isInRangeDay = (d: Date) => {
    if (!d || !tempStartDate || !tempEndDate) return false;
    const t = d.getTime();
    return t > tempStartDate.getTime() && t < tempEndDate.getTime();
  };
  ...
  const monthWeeks = useMemo(() => {
    ...
    for (let startDay = 1; startDay <= daysCount; startDay += 7) {
      ...
      weeks.push({
        ...
        startDate: new Date(pickerYear, pickerMonth, startDay),
        endDate: new Date(pickerYear, pickerMonth, endDay),
      });
    }
    return weeks;
          ...
          <div className="bg-white dark:bg-zinc-900 rounded-2xl shadow-2xl border border-slate-200 dark:border-zinc-800 max-w-2xl w-full p-6 space-y-5 relative text-slate-900 dark:text-zinc-100">
            ...
            {unit === 'weekly' && (
              <div className="space-y-4">
                ...
                <p className="text-xs text-center text-slate-500 font-medium">시작 주 선택 (주 행 클릭)</p>

                <div className="border rounded-xl overflow-hidden divide-y divide-slate-100 dark:divide-zinc-800">
                  {monthWeeks.map((w) => {
                    const isStart = tempStartDate && w.startDate.getTime() === tempStartDate.getTime();
                    const isEnd = tempEndDate && w.endDate.getTime() === tempEndDate.getTime();
                      ...
                      <div
                        ...
                        onClick={() => {
                          if (!tempStartDate || (tempStartDate && tempEndDate)) {
                            setTempStartDate(w.startDate);
                            setTempEndDate(w.endDate);
                          } else {
                            if (w.startDate < tempStartDate) {
                              setTempEndDate(tempStartDate);
                              setTempStartDate(w.startDate);
                            } else {
                              setTempEndDate(w.endDate);
            ...
            {unit === 'monthly' && (
              <div className="space-y-4">
                ...
                <p className="text-xs text-center text-slate-500 font-medium">시작 월을 선택하세요</p>

                <div className="grid grid-cols-3 gap-3 pt-2">
                  {Array.from({ length: 12 }, (_, i) => i).map((mIdx) => {
                    const mDate = new Date(pickerYear, mIdx, 1);
                    const isSelected = tempStartDate && tempStartDate.getMonth() === mIdx && tempStartDate.getFullYear() === pickerYear;
                    return (
                      <button
                        key={mIdx}
                        onClick={() => {
                          if (!tempStartDate || (tempStartDate && tempEndDate)) {
                            setTempStartDate(mDate);
                            setTempEndDate(new Date(pickerYear, mIdx + 1, 0));
                          } else {
                            if (mDate < tempStartDate) {
                              setTempEndDate(tempStartDate);
                              setTempStartDate(mDate);
                            } else {
                              setTempEndDate(new Date(pickerYear, mIdx + 1, 0));
            ...
            {unit === 'yearly' && (
              <div className="space-y-4">
                ...
                <p className="text-xs text-center text-slate-500 font-medium">시작 연도를 선택하세요</p>

                <div className="grid grid-cols-2 gap-3 pt-2">
                  {Array.from({ length: 10 }, (_, i) => pickerDecadeStart + i).map((y) => {
                    const isSelected = tempStartDate && tempStartDate.getFullYear() === y;
                    return (
                      <button
                        key={y}
                        onClick={() => {
