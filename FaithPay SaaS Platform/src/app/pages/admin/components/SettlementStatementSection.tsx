  ...
  const mockPartnerTaxStatements = [
    {
      ...
      grossCommission: 2850000, // 0.15% 수수료
      withholdingTax: 94050, // 3.3% 원천징수
      netPayout: 2755950,
      status: 'ISSUED',
    },
    {
      ...
      businessNo: '920110-1******',
      grossCommission: 950000, // 0.05% 수수료
      withholdingTax: 31350, // 3.3% 원천징수
      netPayout: 918650,
      status: 'ISSUED',
    },
  ];
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {statementType === 'tenant' ? (
        ...
        /* ── 2. 영업 파트너 3.3% 원천징수 명세서 ── */
        <div className="space-y-4">
          {mockPartnerTaxStatements.map((tax) => (
            <div
              key={tax.id}
              className="bg-white dark:bg-zinc-900 rounded-xl border border-slate-200 dark:border-zinc-800 p-5 space-y-4 shadow-2xs"
            >
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between border-b border-slate-100 dark:border-zinc-800 pb-3 gap-2">
                <div>
                  <span className="text-[11px] font-mono text-slate-400">발행 번호: {tax.id}</span>
                  <h3 className="font-bold text-slate-900 dark:text-zinc-100 text-base flex items-center gap-2">
                    {tax.partnerName} <span className="text-xs font-normal text-slate-500">({tax.month} 정산)</span>
                  </h3>
                  <p className="text-xs text-slate-500">사업자/주민등록번호: {tax.businessNo}</p>
                </div>
                <div className="flex gap-2">
                  <button
                    onClick={() => handlePrint(`${tax.partnerName} 원천징수 영수증`)}
                    className="px-3 py-1.5 text-xs font-semibold bg-slate-100 hover:bg-slate-200 text-slate-800 rounded-lg flex items-center gap-1.5 cursor-pointer border-none"
                  >
                    <Printer className="h-3.5 w-3.5" /> 인쇄
                  </button>
                  <button
                    onClick={() => handleDownloadPDF(`${tax.partnerName} 3.3% 원천징수 영수증`)}
                    className="px-3 py-1.5 text-xs font-semibold bg-emerald-50 hover:bg-emerald-100 text-emerald-800 rounded-lg flex items-center gap-1.5 cursor-pointer border-none"
                  >
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 text-xs">
                <div className="bg-slate-50 dark:bg-zinc-800/50 p-3 rounded-lg">
                  <span className="text-[11px] text-slate-500 block mb-0.5">영업 수수료 총액</span>
                  <span className="font-bold font-mono text-slate-900 dark:text-zinc-100 text-sm">
                    {tax.grossCommission.toLocaleString()}원
                  </span>
                </div>
                <div className="bg-slate-50 dark:bg-zinc-800/50 p-3 rounded-lg">
